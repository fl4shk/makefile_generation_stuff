...
dnl
dnl ifelse(WHICH_HDL(), `verilog', `BUILD_VVP:=$(VC) $(EXTRA_IVERILOG_OPTIONS) -o $(PROJ).vvp
dnl BUILD_VHDL:=$(VC) $(EXTRA_IVERILOG_OPTIONS) -tvhdl -o $(PROJ).vhd
dnl PREPROCESS:=$(VC) $(EXTRA_IVERILOG_OPTIONS)  -E -o $(PROJ).E'
dnl ,
dnl WHICH_HDL(), `systemverilog', `BUILD_VVP:=$(VC) $(EXTRA_IVERILOG_OPTIONS) -o $(PROJ).vvp
dnl BUILD_VHDL:=$(VC) $(EXTRA_IVERILOG_OPTIONS) -tvhdl -o $(PROJ).vhd
dnl BUILD_VERILOG:=$(VC) $(EXTRA_IVERILOG_OPTIONS) -tvlog95 -o $(PROJ).v
dnl PREPROCESS:=$(VC) $(EXTRA_IVERILOG_OPTIONS) -E -o $(PROJ).E'
dnl ,
dnl `')dnl
OPTIONS_FOR_BUILD_VVP:=$(EXTRA_IVERILOG_OPTIONS)
OPTIONS_FOR_BUILD_VHDL:=$(EXTRA_IVERILOG_OPTIONS) -tvhdl
ifelse(WHICH_HDL(), `verilog', `OPTIONS_FOR_BUILD_FORMAL:=$(EXTRA_IVERILOG_OPTIONS) -DFORMAL'
,
WHICH_HDL(), `systemverilog', `OPTIONS_FOR_BUILD_VERILOG:=$(EXTRA_IVERILOG_OPTIONS) $(EXTRA_BUILD_VERILOG_OPTIONS)
OPTIONS_FOR_BUILD_FORMAL:=$(EXTRA_IVERILOG_OPTIONS) $(EXTRA_BUILD_VERILOG_OPTIONS) -DFORMAL'
,
`')dnl
...
dnl PREPROCESS:=$(VC) $(EXTRA_IVERILOG_OPTIONS)  -E -o $(PROJ).E'
dnl ,
dnl WHICH_HDL(), `systemverilog', `BUILD_VVP:=$(VC) $(OPTIONS_FOR_BUILD_VVP) -o $(PROJ).vvp
dnl BUILD_VHDL:=$(VC) $(OPTIONS_FOR_BUILD_VHDL) -o $(PROJ).vhd
dnl BUILD_VERILOG:=$(VC) $(OPTIONS_FOR_BUILD_VERILOG) -o $(PROJ).v
dnl PREPROCESS:=$(VC) $(OPTIONS_FOR_PREPROCESS) -o $(PROJ).E'
dnl ,
dnl `')dnl
BUILD_VVP:=$(VC) $(OPTIONS_FOR_BUILD_VVP) -o $(PROJ).vvp
BUILD_VHDL:=$(VC) $(OPTIONS_FOR_BUILD_VHDL) -o $(PROJ).vhd
ifelse(WHICH_HDL(), `verilog', `',
WHICH_HDL(), `systemverilog', `BUILD_VERILOG:=$(VC) $(OPTIONS_FOR_BUILD_VERILOG) -o $(PROJ).v'
,
`')dnl
BUILD_FORMAL:=$(VC) $(OPTIONS_FOR_BUILD_FORMAL) -o $(PROJ).v
